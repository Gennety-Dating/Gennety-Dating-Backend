...
function buildPrompt(input: MatchCardCopyInput): string {
  return [
    ...
    `- tagline: one warm line about the person's vibe, max ${TAGLINE_MAX} characters, no name in it.`,
    `- paragraph: max ${PARAGRAPH_MAX} characters, 2 short sentences describing who they are and what being around them feels like. Concrete, warm, grounded in the profile below. Never invent facts.`,
    "- Address the reader informally (ты-form where the language has it).",
