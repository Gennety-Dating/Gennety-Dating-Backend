...
| `* * * * *` | UTC | Telegram-only pinned status banner: **stage-aware** blue countdown button — reply deadline / date / planning for a live match, canonical next-drop otherwise, waitlist copy for an unlaunched city (`resolveBannerStage`, PRODUCT_SPEC §2.1) — plus null/stale-message repair, non-active cleanup; hourly physical-pin reconciliation and 15-minute health heartbeat. **No longer the banner's only writer:** a settled venue change pushes the new render immediately (`services/status-banner-refresh.ts`), because the banner names the venue and a ≤60s lag is glaring on that one screen. The two share `statusBannerRenderCache` and `resolveBannerStage`/`loadBannerStages` (moved into `services/status-banner-stage.ts` for exactly that reason), so the push satisfies the next tick instead of fighting it; creation, pinning, pointer repair and the backoff ladder stay the worker's alone | `workers/status-timer.ts` |
