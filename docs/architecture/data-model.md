...
| Pre-date coordination (feature-flagged) | Live: `proxyOpenedAt`, `proxyClosesAt`, `proxyClosedAt` (+ `proxyReadAtA/B`) — the anonymous chat every scheduled date gets at T-1h; inert when `COORDINATION_FEATURE_ENABLED` is off. **Deprecated 2026-09-26, no longer read or written:** `coordOfferSentAt`, `coordInitiatorId`, `coordMethod` (`share_self`/`request_partner`/`proxy`), `coordChosenAt`, `coordPartnerConsent`, `coordResolvedAt` — the retired T-3h questionnaire (founder decision: no handle exchange); kept so older rows stay intact and no migration was needed. See [PRODUCT_SPEC.md](../product/product-spec.md) §Phase 4. |
...
Indexes: `(status, createdAt)`, `(userAId, userBId)`, `(ticketStatus, ticketExpiresAt)` (ticket-expiry cron sweep), `(status, coordOfferSentAt)` (the retired offer sweep — unused since 2026-09-26), `(coordMethod, proxyClosedAt)` (its `coordMethod` half no longer filters anything since 2026-09-26), `(venueChangeStatus, venueChangeExpiresAt)` (venue-change expiry sweep), plus the functional
...
Append-only audit log of every text message relayed through the
