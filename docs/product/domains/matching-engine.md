...
- **The discount slot is ONE slot.** A famine grant **replaces** whatever is in
  it — a still-starved user who already redeemed one gets a fresh one, TTL slid,
  `consumedAt` cleared. `User.ticketDiscountSource` records which mechanism
