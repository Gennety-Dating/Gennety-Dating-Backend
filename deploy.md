...
**PENDING — голосовой ответ в онбординге идёт под шиммером (PRODUCT_SPEC §1.3,
DECISIONS.md).** **Нет изменения схемы Prisma, нет новых env, нет флагов, нет
изменения Mini App** (`apps/webapp` не тронут) — только бот и `@gennety/shared`,
так что полный деплой кода несёт всё целиком, плюс `pnpm demo:deploy`.

Голосовое, отправленное во время регистрации, теперь держится под двухбитным
`<tg-thinking>`: «🎧 Слушаю твоё голосовое…» (3.5 с) → «💭 Анализирую…» (2 с) →
ответ. После онбординга не меняется ничего.

**Пять вещей, которые стоит знать до рестарта:**

- **Это добавляет секунды в воронку регистрации, и это осознанно.** Скрипт
  ПОКРЫВАЕТ скачивание + Whisper (`until` + `NEVER_CUT_SHORT`), то есть стоимость
  `max(5.5 с, работа)`, а не сумма — но при быстром Whisper это всё равно
  ~2–4 с сверху на каждый голосовой ответ. Смотреть
  `GET /admin/analytics/onboarding-funnel` после деплоя; оба холда — именованные
  константы с потолком, который держит тест.
- **Чат-экшены на этом пути выключаются.** Во время онбординга `record_voice` и
  `typing` больше не отправляются — их заменяет статус. Пост-онбординговый путь
  (каждое голосовое консьержу) байт-в-байт прежний, и это закреплено тестом.
- **Периодический бит «думаю» пропускается для голосового ответа**, но ответ
  засчитывается в каденс. То есть после голосового следующий третий ПЕЧАТНЫЙ
  ответ по-прежнему получает свою паузу.
- **Десять новых ключей i18n** (`voiceAnswerStep1/2` × 5 языков), аддитивно;
  ничего существующего не переименовано. Голосовой промпт §1.3b и его
  `voiceCheckSteps` не тронуты.
- **Проверять на `@gennetytestbot` или в демо** — путь исполняется только внутри
  регистрации, а в проде новых регистраций мало.

Preflight по этому изменению: typecheck чист по 5 проектам, lint чист, **полный
набор бота 4053 теста / 263 файла, 0 failed** (+15 новых) + 280 shared. Четыре
мутации подтверждены **красными** по отдельности: снятый `NEVER_CUT_SHORT`
(1 тест), `narrate = false` (3), чат-экшены всегда включены (1), и снятый гвард
на голосовой ответ в `earnsThinkingPause` (1).

Проверка после деплоя — путь ничего не логирует на счастливой ветке, поэтому
глазами: записать голосовое в ответ на вопрос анкеты и убедиться, что сначала
идут два бита статуса, потом ответ бота, и что «печатает» не появляется под
шиммером. Косвенно:

```sh
pnpm demo:deploy
pm2 logs gennety-bot --lines 200 --nostream | grep -E "Voice download failed|Voice transcription threw"
# Пусто = расшифровка не падает. Строки здесь означают, что пользователь увидел
# «не расслышал» ПОСЛЕ статуса — статус в этом случае корректно снимается первым.
```
