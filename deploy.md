...
четырьмя фотографиями в бандле (`apps/webapp/src/intent/*.webp`, 33–42 кБ
каждая, ~150 кБ суммарно). Без него сервер примет массив, которого некому
прислать.
...
Preflight по этому изменению: typecheck чист (бот + shared + db + video; вебапп
— только чужой незакоммиченный `mascot-welcome.ts`, см. ниже), lint чист,
**бот 4305/4305, shared 328/328, webapp 480/481**. Девять несущих гвардов
...
на экране — плоская заливка поверх разлива, ужатое пятно, поднятые альфы, снятая
задержка подписи, императивный `classList` и нетронутая плитка, играющая уход.

**⚠️ `pnpm typecheck` и полный набор вебаппа КРАСНЫЕ до этой работы и не из-за
неё**: параллельная сессия переписывает `apps/webapp/src/mascot-welcome.ts`
(+390 строк) и уже сняла `handSquash`, который её же тест импортирует. Мои файлы
линтятся и типизируются чисто; 44 из 45 файлов вебаппа зелёные.
...
сноска «видно только тебе», и **несколько плиток отмечаются одновременно**. Тап
должен заливать плитку белым от точки касания — если она белеет мгновенно, уехал
старый бандл. Затем в My Profile — строка «🎯 Ты ищешь: …» ниже блока превью, с
той же подписью, и её редактор тоже переключатель.
