...
- **Экран удлиняется на ~0.9 с** (было ~3.5 с: набор 1.43 с + голый холд
  2.04 с; стало ~4.4 с — холд строки урезан до 0.3 с, дальше 0.26 с паузы и
  2.4 с падения). Онбординговая воронка здесь чувствительна — тот же расчёт,
  из-за которого следят за ~10.7 с «раздумья» Type Radar. Держится тестом
  (`MONEY_VIEW_MS ≤ 2800`), смотреть на `GET /admin/analytics/onboarding-funnel`.
- **Чанк `onboarding` 125.9 → 129.1 kB** (+3.2 kB, ~1 kB gzip). Новых ассетов
...
Preflight: typecheck чист, lint чист, **286 тестов вебаппа** (0 failed, +12
новых), `vite build` чист.
...
# Смотреть на два момента: (1) кадр заполнен купюрами сверху донизу с ПЕРВОГО
# мгновения, а не наполняется сверху занавесом; (2) вопрос читается целиком —
# ни одна купюра не должна смывать строку.
...
**PENDING — на экране метрик каждое число зачёркивает своё приложение
(PRODUCT_SPEC §1.1, DECISIONS.md).** **Нет изменения схемы Prisma, нет новых
env, нет флагов и НЕТ ИЗМЕНЕНИЙ СЕРВЕРА ВООБЩЕ** — диff это `apps/webapp/**`
плюс документация. То есть путь **Deploy Mini App Only**
(`./scripts/deploy-webapp.sh`); рсинкать в `/opt/gennety` нечего, **`pm2 restart`
...
Трей из трёх иконок конкурентов над барабаном метрик был декорацией: числа вели
аргумент, а приложения просто стояли рядом. Теперь метрика `i` убивает иконку
`i` — пока набирается число, иконка трясётся с нарастанием и наливается
бордовым; на последней цифре она полностью красная и тряска обрывается; следом
двумя штрихами прочерчивается крест. После трёх метрик все три перечёркнуты.

**Три вещи, которые стоит знать до редеплоя:**

- **Ничего не удлиняется.** Весь удар укладывается в 1760 мс из 2500 мс шага
  барабана, и это закреплено тестом, а не комментарием: штрих, который ещё
  рисуется, когда пошло следующее число, читается как две иконки, умирающие
  разом. Сам шаг барабана не тронут, длительность экрана прежняя.
- **Чанк `onboarding` вырос 125.89 → 126.40 kB** (+0.5 kB, +0.21 kB gzip), CSS
  56.89 → 60.32 kB (+3.4 kB, +0.69 kB gzip). Новых ассетов нет: покраснение
  берёт маской тот же PNG, который экран уже грузил.
- **Порядок слоёв задан явно (`z-index`).** Убирать его нельзя — у убиваемой
  иконки анимируется `filter`, Chromium поднимает такой элемент в отдельный слой
  композитора, и картинка возвращается поверх красного. Найдено замером на живой
  странице: две простаивающие иконки краснели, а та, ради которой всё написано,
  — нет.

Проверка после редеплоя — экран транзиентный и ничего не логирует, поэтому
глазами. Продовый онбординг малотрафиковый, так что смотреть на
`@gennetytestbot` (нужен HTTPS-туннель и `WEBAPP_URL` на него) либо на
дев-превью, которому не нужны ни Telegram, ни аккаунт:

```sh
./scripts/deploy-webapp.sh
pnpm demo:deploy
curl -sI https://dating-calendar.gennety.com/onboarding.html | head -1
# Дев-превью экрана метрик, обе темы:
#   http://localhost:5173/onboarding.html?preview=intro:3&lang=ru&theme=dark
#   http://localhost:5173/onboarding.html?preview=intro:3&lang=ru&theme=light
# Смотреть на три момента: (1) тряска нарастает, а не качается ровно, и
# обрывается ровно на последней цифре; (2) крест виден на бордовом силуэте —
# если линии сливаются с иконкой, значит кто-то свёл два красных в один;
# (3) через полный круг (~7.5с) перечёркнуты все три и больше ничего не
# переигрывается.
```
