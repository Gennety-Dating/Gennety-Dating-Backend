...
(§1.3b) и `profiles.relationship_intent` + `match_score_logs.score_intent`
...
Две новые колонки — `profiles.relationship_intent` (nullable) и
...
порядок ломается: новый бандл увидит `relationshipIntent: undefined`, покажет
экран, а старый сервер не разберёт поле в `POST /profile` и ответит
**`400 no-fields`** — то есть пользователь упрётся в экран, который невозможно
пройти.
...
- **Существующие аккаунты не трогаются.** Колонка nullable, а `intentMultiplier`
  возвращает ровно 1.0, когда её нет у любой стороны, так что ни один legacy-
  профиль не штрафуется. Задать значение они могут через новую строку в
  My Profile.
...
набор 4739 тестов** (бот 4065 / shared 293 / webapp 381), `pnpm build` чист,
`openapi:lint` valid. Три несущих гварда подтверждены **красными** по
отдельности: снятый `vIntent` из произведения, штраф за отсутствующее намерение,
и `intent`, выпавший из `composeScore`.

Проверка после деплоя — счастливый путь ничего не логирует, поэтому по схеме и
глазами:

```sh
psql "$DATABASE_URL" -c "select relationship_intent, count(*) from profiles group by 1;"
# Всё NULL сразу после деплоя; значения появляются по мере регистраций.
...
после экрана роста идёт «что ты ищешь?» с четырьмя строками в ОДНОМ тоне и
сноской «видно только тебе», а затем в My Profile — строка «🎯 Ты ищешь: …» ниже
блока превью, с той же подписью.
