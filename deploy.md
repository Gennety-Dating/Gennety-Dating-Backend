...
**Восемь вещей, которые стоит знать до редеплоя:**
...
- **Стоимость воронки: +4.7 с приветствия и пол цикла 2.0 с**, и только на
  первом экране, только один раз. Оба числа приколочены тестом (`GREET_MS ≤
  4800`, `LOOP_FLOOR_MS ≤ 2200`) — как `SUCCESS_TOTAL_MS`. Смотреть
  `GET /admin/analytics/onboarding-funnel` на шаге `language` после деплоя.
  **Потолки подняты сознательно 2026-08-26** (было 3800/1500) — жест «взял,
  рассмотрел, вернул» физически длиннее тапа, а пол лупа теперь измерен в
  ЖЕСТАХ, а не выбран круглым числом. Это решение, а не дрейф; следующее
  повышение обязано быть таким же явным.
- **Чанк онбординга 112 → 130.4 kB** (gzip 43.5 kB), CSS 45.7 → 49.6 kB. Новых
  ассетов нет — маскот целиком векторный и живёт в чанке страницы. (Кисть-круг,
  заменившая перчатку 2026-08-23, чанк не увеличила, а **уменьшила** на 1.4 kB
  — замерено сборкой обеих версий, а не предположено. Переписанный жест
  2026-08-26 добавил 3.0 kB.)
...
Preflight по этому изменению: typecheck чист, lint чист, **488 тестов вебаппа**
(45 файлов, +49 в `mascot-welcome.test.ts`), `vite build` чист. Пять несущих
гвардов подтверждены красными по отдельности: снятая проверка силуэта роняет
тест целей; константный угол манжеты — два теста касательной; возврат прежних
кривых разворота — тест «не больше пятой части размаха за кадр»; константное
смещение хвата — тест «не накрывает карточку, которую держит»; снятое
ограничение возврата — тест «берёт только то, что может вернуть на сцену».
...
# Смотреть на пять моментов: (1) поднятая карточка целиком видна и заметно
