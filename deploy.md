...
Preflight по этому изменению: typecheck чист (единственная ошибка —
`src/__preview.test.ts`, untracked-файл параллельной сессии, `node:fs` без
`@types/node`; к этому изменению отношения не имеет), lint чист, **332 теста
вебаппа зелёные** (+7 новых), `vite build` чист. Главный гвард подтверждён
...
**PENDING — единая марка успеха на всех экранах: бабочка чертит галочку и
улетает (PRODUCT_SPEC → Cross-Cutting Concerns, DECISIONS.md ×2).** **Нет
изменения схемы Prisma, нет новых env, нет флагов и НЕТ ИЗМЕНЕНИЙ СЕРВЕРА
ВООБЩЕ** — диff это `apps/webapp/**` плюс документация. То есть путь **Deploy
...
собирает свой бандл из того же исходника и иначе останется на старом.

Четыре разные галочки на экранах успеха (зелёный диск в verification, 84px обводка
в календаре, белый диск с **чёрной** галочкой в Type Radar, бордовый градиентный
диск в venue-change) заменены одной общей маркой. Пятый экран — `DoneScene`
онбординга — показывал `loading-orb`, то есть спиннер загрузки на завершённом
состоянии, и тоже получил марку.

**Марка переработана 2026-08-13 по отзыву founder'а, до того как что-либо уехало:**
бабочка больше не садится на кончик галочки, а **уходит из кадра**, оставляя
чистую жирную бордовую галочку. Ниже — состояние после этой переработки; блок
описывает один деплой, а не два.

**Семь вещей, которые стоит знать до редеплоя:**

- **Меняется тайминг двух самозакрывающихся экранов, и в обе стороны.**
  Verification: было 2200 мс от ответа сервера, стало «дождаться покоя марки от её
  собственного монтирования + 1100 мс» — при быстром ответе это ~2300 мс (чуть
  дольше), при медленном короче, потому что марка уже отвиселась. Type Radar: было
  2100 мс, стало 2300 мс. Обе задержки выведены из анимации, так что удлинить
  полёт и не заметить закрытие поверх недорисованной галочки нельзя.
- **Хаптик переехал на момент, когда штрих замыкается** (900 мс), а НЕ на конец
  ухода: уход — это инструмент, покидающий кадр, и жужжать о нём значит жужжать не
  о результате. В verification и календаре импульс срабатывал ДО того, как марка
  была нарисована. Онбординг получает импульс, которого у него не было вовсе. При
  `prefers-reduced-motion` импульс срабатывает сразу — марка там нарисована уже
  завершённой.
- **Venue-change сознательно НЕ получает второй импульс:** этот экран достигается
  либо после оплаты Stars (которая уже жужжит на «paid»), либо повторным открытием
  уже закреплённой доски, где не произошло ничего.
- **Вся анимация теперь 1200 мс** (было 1280): 900 мс рисование + 300 мс уход, и
  уход, свечение (720+480) и подпись (820+380) приходят в покой на одном кадре.
- **При `prefers-reduced-motion` бабочки нет вовсе** — только готовая галочка и
  свечение, плюс короткое проявление. Это следствие того, что базовые значения CSS
  и есть финальные, так что медиазапрос не переписывает ни одной позы.
- **Удалён токен `--success: #28c76f`** — последний зелёный в продукте, его больше
  не читает никто. И ~115 строк мёртвого CSS в четырёх файлах.
- **Бандл: +2.0 kB gzip** (`butterfly-success` JS 0.86 + CSS 0.93 + React-обёртка
  0.21), минус удалённый CSS. Геометрия логотипа вынесена в общий
  `brand-butterfly.ts`, который теперь читают обе марки; вывод лоадера при этом
  **байт-в-байт прежний** — это держит существующий тест на идентичность с
  инлайновой копией в `verification.html`.

Preflight по этому изменению: typecheck чист, lint чист, **324 теста webapp**
(29 файлов, 0 failed; +38 новых: 37 в файле марки, 1 в verification), `vite build`
чист. Четыре гварда подтверждены красным до фикса: рассинхрон стопов полёта и
штриха, расхождение взмаха крыльев с лоадером, база позы против последнего
кейфрейма, и выход повёрнутой бабочки за кадр (последний ловит ровно тот дефект,
который прошёл мимо предыдущей версии проверки).

Проверка после редеплоя — экраны транзиентные и ничего не логируют, поэтому
глазами. Продовый трафик здесь редкий (0 свиданий за всю историю, регистраций
мало), так что смотреть на `@gennetytestbot` или в демо:

```sh
./scripts/deploy-webapp.sh
pnpm demo:deploy
for p in index verification radar venue-change onboarding; do
  curl -sI "https://dating-calendar.gennety.com/${p/index/}" | head -1
done
# Смотреть на четыре момента на каждом из пяти экранов:
#   (1) ФИНАЛЬНЫЙ КАДР — только галочка, бабочки на ней НЕТ. Если логотип остался
#       сидеть на острие, уехал старый бандл;
#   (2) галочка бордовая, а не розовая, и стоит по центру своего места;
#   (3) в полёте бабочка идёт ВПЕРЁД штриха, а не по нему — штрих должен выходить
#       у неё из-под хвоста, а не сквозь неё;
#   (4) вибрация приходит в момент, когда галочка дочерчена, а не позже.
# Самый быстрый путь: в демо дойти до назначенного свидания (venue-change settled)
# и до финального экрана онбординга.
```
