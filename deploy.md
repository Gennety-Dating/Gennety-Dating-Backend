...
**Семь вещей, которые стоит знать до редеплоя:**
...
- **Стоимость воронки: +3.7 с приветствия и пол цикла 1.4 с**, и только на
  первом экране, только один раз. Оба числа приколочены тестом (`GREET_MS ≤
  3800`, `LOOP_FLOOR_MS ≤ 1500`) — как `SUCCESS_TOTAL_MS`. Смотреть
  `GET /admin/analytics/onboarding-funnel` на шаге `language` после деплоя.
- **Чанк онбординга 112 → 127.3 kB** (gzip 42.5 kB), CSS 45.7 → 46.7 kB. Новых
  ...
  — замерено сборкой обеих версий, а не предположено.)
...
Preflight по этому изменению: typecheck чист по 5 проектам, lint чист, **378
тестов вебаппа** (36 файлов, +31 новый), `vite build` чист. Оба несущих гварда
подтверждены красными по отдельности: снятая проверка силуэта роняет тест
целей, константный угол манжеты — два теста касательной.
...
#   /onboarding.html?preview=welcome:loop:700        — кадр цикла
#   /onboarding.html?preview=welcome:greet:3150      — кадр шторы
# Смотреть на четыре момента: (1) в цикле руки берут карточки СБОКУ и ни одна
# не ложится на тело; (2) ладонь доворачивается вместе с рукой, а не едет
# наклейкой; (3) сквозь уходящую штору виден настоящий экран, а не чёрное;
# (4) на светлой теме перчатки видны (у них своя тень).
```
