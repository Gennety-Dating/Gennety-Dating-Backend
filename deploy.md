...
**Порядок: сервер первым.** Дашборд читает `.catch(() => null)` по секции, так
что старый фронт против нового сервера просто не рисует блок; обратный порядок
даёт живую вкладку, зовущую 404.
