...
**PENDING — единая марка успеха на всех экранах: бабочка чертит галочку
...
- **Меняется тайминг двух самозакрывающихся экранов, и в обе стороны.**
  ...
  собственного монтирования + 1100 мс» — при быстром ответе это ~2380 мс (чуть
  дольше), при медленном короче, потому что марка уже отвиселась. Type Radar: было
  2100 мс, стало 2380 мс. Обе задержки теперь выведены из анимации, так что
  удлинить полёт и не заметить закрытие поверх недорисованной галочки нельзя.
- **Хаптик переехал на момент приземления.** В verification и календаре он
  срабатывал ДО того, как марка была нарисована. Онбординг получает импульс,
  которого у него не было вовсе. При `prefers-reduced-motion` импульс срабатывает
  сразу — марка там нарисована уже завершённой.
...
Preflight по этому изменению: typecheck чист, lint чист, **319 тестов webapp**
(0 failed, +33 новых: 32 в новом файле марки, 1 в verification), `vite build` чист. Два главных гварда подтверждены красным
до фикса: рассинхрон стопов полёта и штриха, и расхождение взмаха крыльев с
лоадером.
...
# Смотреть на три момента на каждом из пяти экранов:
#   (1) кончик штриха всё время под бабочкой — если она летит рядом со своим
#       штрихом, разъехались стопы кейфрейма;
#   (2) финальный кадр — бабочка СТОИТ РОВНО на кончике, не наклонена;
#   (3) вибрация приходит в момент приземления, а не раньше картинки.
