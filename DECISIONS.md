...
**Открытый пункт, названный вслух:** английского `tg-open` не существует —
записи опускают «домашний экран → Telegram → Gennety в списке → Start». Это
последние 5 секунд фильма; остальные семнадцать шотов строятся без него. Запрос
на доп-запись сформулирован в §4.1 брифа.
