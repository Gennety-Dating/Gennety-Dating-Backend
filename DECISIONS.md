...
## 2026-08-12 — иконки на экране метрик зачёркиваются: два красных, а не один

**Kind:** deviation from plan
**What:** каждая метрика на экране статистики убивает иконку под собой — тряска
с нарастанием, покраснение, крест. Просили «красные линии» на иконке, которая к
тому моменту «полностью красная»; сделано двумя красными, разведёнными по
светлоте: бордовое тело (`--accent`), яркий скарлет у линий.
**Why:** буквальное исполнение невозможно, и это не вкусовщина. Иконка в конце —
сплошной красный силуэт; линии того же красного на нём не видны вообще, то есть
финальный кадр анимации оказался бы пустым. Разведение по светлоте сохраняет обе
части просьбы («красный» и «красные линии») и оставляет крест читаемым; линии
дополнительно несут тёмную тень, потому что на светлой теме скарлет ложится на
бледный стеклянный трей.
**What it changes going forward — три вещи, которые легко сломать обратно:**
- **Порядок слоёв задан явно (`z-index` 0/1/2), и это не перестраховка.**
  Найдено замером, а не рассуждением: две простаивающие иконки краснели
  правильно, а та, которую реально убивают, — нет. Причина в том, что у неё
  анимируется `filter` (обесцвечивание), Chromium поднимает такой элемент в
  отдельный слой композитора, и картинка возвращается поверх красного. Убрать
  `z-index` «как лишний» — значит вернуть баг ровно на той иконке, ради которой
  всё написано.
- **Ускорение живёт в РАССТАНОВКЕ стопов кейфрейма, а не в easing.** Нули стоят
  на `(k/13)^1.7`, амплитуда растёт `(k/13)^1.2`. Плавная кривая при постоянной
  частоте разгоняет и тормозит обратно — это качание, а не «теряет опору».
  Последний стоп возвращает в ноль: тряска обрывается на кадре, где число
  дописалось.
- **Длительность счётчика теперь одна на всех** (`onboarding-kill.ts`). До этого
  `1250` было зашито внутри `CountUpText`; рампа обязана кончаться ровно на
  последней цифре, и вторая копия числа разъехалась бы при первой правке
  незаметно для диффа. Там же тестом закреплено, что весь удар помещается в один
  шаг барабана.
**Одна правка по замеру, а не по вкусу:** вылет линий за края уменьшен с 6% до
3%. На 390px зазор в трее упирается в `4vw` (~15px), и при 6% три креста
смыкались в одну зигзагообразную полосу поперёк трея.
**Recorded in:** PRODUCT_SPEC.md §1.1, `apps/webapp/src/onboarding-kill.ts`.
