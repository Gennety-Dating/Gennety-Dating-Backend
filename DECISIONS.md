...
**Размеры не скопированы дословно с широких пиллов.** Смещения — примерно шестая
часть диаметра (56px), иначе свет с четырёх сторон встречается в середине и
кружок вырождается в равномерно более светлый диск, то есть эффект края
пропадает именно там, где он единственный носитель формы. Проверено рендером в
обеих темах на обеих поверхностях: под фото-героем «как это работает» (тема
страницы) и над всегда-тёмным размытым фото в `.howitworks-dock`.
