...
## 2026-08-23 — растворение было привязано не к тому краю; и гвард, который подтвердил баг

**Kind:** deviation from plan
**What:** растушёвка кадровых границ перенесена в подготовку ассета
(`prepare.mjs`) — маска кнопки осталась как была.
**Why:** фаундер сообщил, что у парня справа переход отличный, а слева плечо
обрывается прямой линией; у девушки так же по левому краю с рукой. Асимметрия и
была подсказкой, и я сначала прочитал жалобу как «маска слишком узкая», что
неверно.

**Маска привязана к краю КНОПКИ, а обрывы — это край исходной ФОТОГРАФИИ, и они
не совпадают.** Посчитано по задеплоенным ассетам: контент парня занимает
x 61…558 из 540, девушки — 60…408. Справа у парня рисунок вылезает за кнопку,
там маска доводит его до нуля — отсюда «справа круто». Слева его плечо кончается
на 11% ширины, где маска держит ещё ~77% непрозрачности, и кадровая граница
видна почти во всю силу. У девушки **обе** вертикали искусственные и лежат
внутри кнопки: слева рамка кадра режет руку, справа мой же отсекатель камеры
режет волосы.

**What it changes going forward:**
- **Разделение обязанностей:** ассет отвечает за свои кадровые границы, кнопка —
  за свой край. Край, вылезающий за кнопку, ассет НЕ трогает, иначе он
  растворяется дважды.
- **Растушёвка живёт в скрипте, а не в CSS**, потому что она зависит от того,
  где именно оказался рисунок, а это знает только подготовка.
- **Числа геометрии печатаются на каждом прогоне** (`content x 61..558, left
  feathered, right clipped by button`) — новая фотография сразу показывает, что
  с ней произошло, вместо того чтобы это выяснялось на телефоне.

**Отдельная запись про метод, потому что это стоило времени.** Я написал гвард:
максимальный скачок средней альфы между соседними колонками, порог 20%. Он
**прошёл при растушёвке, сведённой к 1%**, то есть дал зелёный свет ровно тому
дефекту, ради которого писался. Причина не в пороге, а в постановке: у выреза
силуэт резкий по всему контуру — это и есть вырез, — а кадровую границу отличает
только то, что она прямая и длинная, чего усреднение по колонке не видит. Гвард
удалён: тот, что подтверждает баг, хуже отсутствующего, потому что создаёт
ложную уверенность. Проверка здесь — рендер в реальный размер.
